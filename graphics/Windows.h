...
/* returns a pointer to the button on the window by its type
 * returns NULL if there isn't Button of type "type" in wndw*/
Button *get_button_by_type(Window* wndw, ButtonType type);

/* helping function for create_window
 * get num of buttons by window type */
int num_buttons_by_window_type(Window_type type);

/* helping function for create_window
 * get pointer to button creator by window type */
buttons_creator button_creator_by_window_type(Window_type type);

/* helping function for create_window
 * get window name by window type */
